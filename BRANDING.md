...
| `og/blog/*.png` | Per-post share cards, generated by `scripts/generate-og-images.ts` from post titles | Generated at build. Carry `Blog — Matt Wilson` with an em dash |
| `headshot.png` | Used by the About component | Component is unrendered — see [Open](#open) |
| `linkedin-cover.png` / `.svg` | LinkedIn banner | 🔴 He is not happy with how it reads. Gates the launch |
| `matt-wilson-resume-phantom.*` | Résumé built for the Phantom application | Not brand material. Served publicly from `public/` — decide whether it should be |
| `site.webmanifest` | PWA manifest | 🔴 **Stale.** `name` is `Matt Wilson — Full Stack Engineer` — the employability title, plus the banned em dash. `background_color` is `#ffffff` on a dark-only site |
...
| `llms.txt` | His own idea — the "Who are we building for?" post practising what it argues |
